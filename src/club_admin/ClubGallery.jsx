...
function ClubGalleryRoute() {
    ...
    const getMemberGallery = async () => {
        try {
            const response = await fetch(`${API_URL}/members-galleries`, {
                headers: {
    ...
    const getClubGallery = async () => {
        try {
            const response = await fetch(`${API_URL}/club-gallery`, {
                headers: {
                    Accept: "application/json",
                    Authorization: `Bearer ${localStorage.getItem("token")}`,
                },
    ...
    const memberImages = useMemo(() => {
        return memberGallery.reduce(
            (sum, member) => sum + Number(member?.gallery_total_photos || 0),
            0
        );
    }, [memberGallery]);
    ...
    const memberLikes = useMemo(() => {
        return memberGallery.reduce(
            (sum, member) => sum + Number(member?.gallery_total_likes || 0),
            0
        );
    }, [memberGallery]);
        ...
        return clubGallery.reduce(
            (galleryTotal, gallery) => {
                const photoLikes = gallery?.photos?.reduce(
                    ...
                    0) || 0;
                return (
                    galleryTotal + Number(gallery?.total_likes ?? photoLikes)
                );
            },
            0
        );
    ...
    const memberComments = useMemo(() => {
        return memberGallery.reduce(
            (sum, member) => sum + Number(member?.gallery_total_comments || 0),
            0
        );
    }, [memberGallery]);
        ...
        return clubGallery.reduce(
            (galleryTotal, gallery) => {
                const photoComments = gallery?.photos?.reduce(
                    ...
                    0) || 0;
                return (
                    galleryTotal + Number(gallery?.total_comments ?? photoComments)
                );
            },
            0
        );
    ...
    const totalPages = Math.max(1, Math.ceil(filteredClubGalleries.length / itemsPerPage));
...
export default ClubGalleryRoute;