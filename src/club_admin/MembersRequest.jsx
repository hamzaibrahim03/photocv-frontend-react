...
const MembersRequest = () => {
    ...
    useEffect(() => {
        ...
        const fetchMembers = async () => {
            try {
                // Fake data representing member data structure
                const dummyData = [
                    // Member structure sample: 
                    // { username, tag_line, profile_image_url, gallery_total_photos, gallery_total_comments, gallery_total_likes, galleries: [{photos: [{image_url}], gallery_name}]}
                ];
    ...
    return (
        <div className="content" style={{ backgroundColor: '#f9f9f9', minHeight: '100vh', padding: '0 30px' }}>
            <NavigationRoute />
            ...
            <section>
                <div className="container" style={{ maxWidth: '1810px', margin: '0 auto', padding: '0 15px' }}>
                    {chunkedCards.length > 0 ? chunkedCards.map((row, rowIndex) => (
                        <div key={rowIndex} className="row-wrapper d-flex mb-4">
                            <div className="photographers d-flex w-100 gap-4 flex-wrap mt-2">
                                {row.map((member, index) => (
                                    <div key={index} className="photographer-card bg-white text-center rounded shadow-sm border p-3" style={{ width: '23.6%', borderColor: '#99816b' }}>
                                        <img src={member.profile_image_url || '/placeholder.jpg'} alt="Profile" className="profile-pic rounded-circle mx-auto d-block my-3" style={{ width: '192px', height: '192px', objectFit: 'cover' }} />
                                        ...
                                        <small className="role text-muted d-block mb-3">{member.tag_line || 'No tagline provided'}</small>
                        ...
                        <p className="text-muted text-center pt-5">No members found.</p>
                    )}
                </div>
            </section>
                ...
                <div className="container" style={{ maxWidth: '1810px', margin: '0 auto', padding: '0 15px' }}>
                    <div className="d-flex justify-content-between align-items-center mb-4">
                        <div className="button-group ms-3">
                            <button className="btn text-white" style={{ backgroundColor: '#4c4036', width: '120px' }} onClick={() => navigate('/add-new')}>Add New</button>
                        </div>
